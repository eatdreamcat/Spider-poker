  ...
  GameScene: [ function(require, module, exports) {
    ...
    var GameScene = function(_super) {
      ...
      GameScene.prototype.startGame = function() {
        ...
        var APokers = pokers.splice(0, 8);
        console.log(pokers);
        console.log(pokers.length);
        var PokerChildren = [];
        var posNoA = [ 2, 3, 4, 5, 6, 9, 10, 11, 12, 15, 16, 17, 20, 21, 24, 28, 29, 32, 35, 38, 41, 44, 47 ];
        var posOther = [ 0, 1, 7, 8, 13, 14, 18, 19, 22, 23, 25, 26, 27, 30, 31, 33, 34, 36, 37, 39, 40, 42, 43, 45, 46, 48, 49, 50, 51 ];
        console.log(" TotalLengt:", posNoA.concat().length + posOther.concat().length);
        while (pokers.length > 0) {
          ...
          var indexRandom = CMath.getRandom(0, 1);
          var index = 0, pos = 0;
          if (posNoA.length > 0) {
            index = Math.floor(indexRandom * posNoA.length);
            pos = posNoA.splice(index, 1)[0];
          } else {
            index = Math.floor(indexRandom * posOther.length);
            pos = posOther.splice(index, 1)[0];
          }
          PokerChildren[pos] && console.error(" \u51fa\u9519\u4e86\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01");
          PokerChildren[pos] = pokerNode;
        }
        while (APokers.length > 0) {
          var totalWeight = APokers.length;
          var random = CMath.getRandom(0, 1);
          var randomIndex = Math.floor(random * totalWeight);
          var i = APokers.splice(randomIndex, 1);
          var pokerNode = GameFactory_1.gFactory.getPoker(i);
          var indexRandom = CMath.getRandom(0, 1);
          var index = Math.floor(indexRandom * posOther.length);
          var pos = posOther.splice(index, 1)[0];
          PokerChildren[pos] && console.error(" \u51fa\u9519\u4e86\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01\uff01");
          PokerChildren[pos] = pokerNode;
        }
        console.log(PokerChildren.concat().length);
        console.log(PokerChildren.concat());
        while (PokerChildren.length > 0) {
          var name = (PokerChildren.length - 1).toString();
          var pokerNode = PokerChildren.shift();
          pokerNode.name = name;
          pokerNode.x = 0;
          pokerNode.y = 0;
          console.log(pokerNode.name);
        ...
        var func1 = function() {
          ...
          if (!pokerNode) {
            console.error(" poker node invaild:", totalCount - count);
            return;
          }
  ...
  Poker: [ function(require, module, exports) {
    ...
    var Poker = function(_super) {
      ...
      Poker.prototype.autoCompleteDone = function() {
        var _this = this;
        var time = 0;
        time = Math.max(0, this.recycleActionInfo.duration - (Date.now() - this.recycleActionInfo.startTime));
        time += 100;
        ...
        this.scheduleOnce(function() {
          var dir = (_this.value % 2 == 1, 1);
          var offsetX = CMath.getRandom(0, 2);
          _this.canMove = false;
          _this.node.runAction(cc.sequence(cc.delayTime((13 - _this.value) / 10 + CMath.getRandom(0, 2)), cc.callFunc(function() {
      ...
      Poker.prototype.checkAutoRecycle = function() {
        ...
        index >= 0 && this.placeToNewCycleNode(index);
        return index >= 0;
      };
