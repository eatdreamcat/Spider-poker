window.__require = function e(t, n, r) {
    ...
    var AudioController = function() {
      ...
      AudioController.prototype.initEvent = function() {
        ...
        EventManager_1.gEventMgr.on(EventName_1.GlobalEvent.SMALL_BGM, function() {
          null != _this.audioID["bgm"] && cc.audioEngine.setVolume(_this.audioID["bgm"], .6);
        }, this);
        EventManager_1.gEventMgr.on(EventName_1.GlobalEvent.NORMAL_BGM, function() {
          null != _this.audioID["bgm"] && cc.audioEngine.setVolume(_this.audioID["bgm"], 1.2);
        }, this);
        EventManager_1.gEventMgr.on(EventName_1.GlobalEvent.PLAY_RECYCLE_POKERS, function() {
          _this.audioID["recyclePoker"] = _this.play("recyclePoker");
          cc.audioEngine.setFinishCallback(_this.audioID["recyclePoker"], function() {
  ...
  EventName: [ function(require, module, exports) {
    ...
    (function(GlobalEvent) {
      ...
      GlobalEvent[GlobalEvent["PLAY_OVER"] = 11] = "PLAY_OVER";
      GlobalEvent[GlobalEvent["PLAY_PAUSE"] = 12] = "PLAY_PAUSE";
      GlobalEvent[GlobalEvent["PLAY_START"] = 13] = "PLAY_START";
      GlobalEvent[GlobalEvent["PLAY_SHAKE"] = 14] = "PLAY_SHAKE";
      GlobalEvent[GlobalEvent["SMALL_BGM"] = 15] = "SMALL_BGM";
      GlobalEvent[GlobalEvent["NORMAL_BGM"] = 16] = "NORMAL_BGM";
      GlobalEvent[GlobalEvent["PLAY_RECYCLE"] = 17] = "PLAY_RECYCLE";
  ...
  Poker: [ function(require, module, exports) {
    ...
    var Poker = function(_super) {
      ...
      Poker.prototype.initEvent = function() {
        ...
        EventManager_1.gEventMgr.once(EventName_1.GlobalEvent.AUTO_COMPLETE_DONE, this.autoCompleteDone, this);
      };
      Poker.prototype.autoCompleteDone = function() {
        ...
        var time = 0;
        action && !action.isDone() && (time = Math.max(0, this.recycleActionInfo.duration - (Date.now() - this.recycleActionInfo.startTime)));
        time += .05;
      ...
      Poker.prototype.checkAutoRecycle = function(delay) {
        ...
        var index = -1;
        window["CheatOpen"] ? index = Poker_1.DebugRecycIndex++ % 4 : Game_1.Game.getCycledPokerRoot().forEach(function(key, node) {
          var poker = node.getComponent(Poker_1);
          poker ? Poker_1.checkRecycled(poker, _this) && (index = key) : 1 == _this.value && (index = key);
        });
        index >= 0 && this.placeToNewCycleNode(index, delay);
        return index >= 0;
      };
      ...
      Poker.prototype.checkCanRecycled = function() {
        ...
        Game_1.Game.getCycledPokerRoot().forEach(function(key, root) {
          var poker = root.getComponent(Poker_1);
          if ((_this.node.name != root.name || !poker) && (poker && Poker_1.checkRecycled(poker, _this) || !poker && 1 == _this.value)) {
            var pos = CMath.ConvertToNodeSpaceAR(root, _this.node.parent);
      ...
      Poker.prototype.updateRootNode = function(index) {
        ...
        if (this.node.childrenCount <= this.defualtChildCount) Game_1.Game.addPlacePokerRoot(index, this.node); else {
          if (!this.next) return;
      ...
      Poker.prototype.placeToNewCycleNode = function(index, delay) {
        ...
        var action = cc.sequence(cc.delayTime(delay), cc.callFunc(function() {
          EventManager_1.gEventMgr.emit(EventName_1.GlobalEvent.DEV_POKERS);
          EventManager_1.gEventMgr.emit(EventName_1.GlobalEvent.PLAY_RECYCLE);
        }), cc.moveTo(time, 0, 0), cc.callFunc(function() {
          _this.node.group = "default";
      ...
      Poker.prototype.onAddChild = function(child) {
        ...
        if (this.cycled) {
          ...
          if (this.forward && this.forward.forward) {
            var forward_1 = this.forward.forward;
            this.scheduleOnce(function() {
              forward_1.frontCard.node.opacity = 0;
            }, .05);
          }
          return;
        }
      ...
      Poker.prototype.onChildRemove = function(child) {
        ...
        if (this.cycled) {
          this.forward && this.forward.forward && (this.forward.forward.frontCard.node.opacity = 255);
          var poker = child.getComponent(Poker_1);
          var index = Game_1.Game.getCycledPokerRoot().keyOf(child);
          null != index && Game_1.Game.addCycledPokerRoot(index, this.node);
          var parentPoker = child.getParent().getComponent(Poker_1);
          poker && (parentPoker && !parentPoker.isCycled() || !parentPoker && "CycleRoot" != child.getParent().getParent().name) && poker.setRecycle(false);
          return;
        }
